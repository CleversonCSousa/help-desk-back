...
public class UpdateTicketStatusUseCase {
    ...
    public void execute(UpdateTicketStatusInput input) {
        User user = this.userRepository.findById(input.userId()).orElseThrow(UserNotFoundException::new);
        Ticket ticket = this.ticketRepository.findById(input.ticketId()).orElseThrow(TicketNotFoundException::new);

        if (user.role() == UserRole.TECHNICIAN) {
            this.technicianIsWhoAssignedToCall(ticket, user.id());
        } else if(user.role() != UserRole.ADMIN) {
            throw new UnauthorizedTicketAccessException();
        }
        Ticket updatedTicket = ticket.setStatus(input.status());
        this.ticketRepository.save(updatedTicket);
    }

    private void technicianIsWhoAssignedToCall(Ticket ticket, UUID technicianId) {
